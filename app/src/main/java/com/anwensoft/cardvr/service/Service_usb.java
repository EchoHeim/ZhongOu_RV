package com.anwensoft.cardvr.service;
...
public class Service_usb extends Service {
    ...
    private final int VendorID=0x0405;
    private final int ProductID_HID=0xA0C0;       //USB_ID
    ...
    private static final String ACTION_USB_PERMISSION = "MacLodge_USB_PERMISSION";//定义常量
    ...
    public void scanUSBDevice(){
        ...
        if (!(deviceList.isEmpty())) //获取的device列表
        {
            for (UsbDevice device: deviceList.values())
            {
                ...
                if(usbDevice.getProductId()==ProductID_HID&&usbDevice.getVendorId()==VendorID)     //判断usb设备P_ID V_ID
                {
                    ProductID = 1;       //与p_id v_id吻合  判断usb设备
                    Toast.makeText(this, "success", Toast.LENGTH_SHORT).show();
                    usbManager.requestPermission(usbDevice, intent);
                }
                else
                    ProductID=2;       //不匹配  判断usb设备为第三方设备

                if (usbManager.hasPermission(usbDevice))//判断是否拥有访问权限
                {
